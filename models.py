"""
Core data models for the Research Email Multi-Agent System.
"""
...
class TavilySearchResult(BaseModel):
    """Model for individual Tavily search results."""
    ...
    class Config:
        ...
        json_schema_extra = {
            "example": {
                ...
                "content": "A comprehensive guide to AI safety principles...",
                "score": 0.95
            }
        }
...
class AgentDelegationRequest(BaseModel):
    ...
    target_agent: str = Field(..., description="Target agent name (email_agent)")
    prompt: str = Field(..., description="Prompt for the target agent")
    context: Optional[Dict[str, Any]] = Field(None, description="Additional context for delegation")
...
class AgentDelegationResponse(BaseModel):
    ...
    agent_response: str = Field(..., description="Response from the target agent")
    target_agent: str = Field(..., description="Name of the target agent")
    timestamp: datetime = Field(default_factory=datetime.now, description="Delegation timestamp")